...
import { readGroups, resolveGroupsFile } from '../../../server/groupsFile.ts';
...
const groupsFile = resolveGroupsFile();
const hasLibrary = !process.env.CHEMICAL_GROUPS_FILE && existsSync(groupsFile);
const skipWithoutLibrary = { skip: !hasLibrary };

const groups = hasLibrary ? readGroups(groupsFile) : [];
const duplicates = getDuplicateSymbols(groups);

test('no group of the data carries an error', skipWithoutLibrary, () => {
  const errors: string[] = [];
...
test(
  'the warnings of the data are the ones that are known',
  skipWithoutLibrary,
  () => {
